...
const Carousel = ({ cardData }) => {
  ...
  return (
    <div onTouchStart={onTouchStartEvent} onTouchMove={onTouchMoveEvent} onTouchEnd={onSwipeEvent} className={styles.relativeContainer}>
      ...
      <img
        data-testid={leftBtnTestId}
        src={arrLeft}
        onClick={() => dispatch({ type: types.PREV })}
        className={`${styles.preBtn} ${styles.btns}`}
        alt="arrowLeft"
      />
      <img
        data-testid={rightBtnTestId}
        src={arrRight}
        onClick={() => dispatch({ type: types.NEXT })}
        className={`${styles.nextBtn} ${styles.btns}`}
        alt="arrowRight"
      />
